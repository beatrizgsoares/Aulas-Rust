...
fn main() {
    ...
    //println!("{:?}",lista_isbns);
    println!("Livros que têm a palavra-chave: 'português' e 'romance':");
    for id in lista_ids{
        println!("{}", livraria.objeto_por_id.get(&id).unwrap());
    }
...
impl Livraria{
    ...
    fn procurar_por_titulo(&self, titulo:&str) -> Vec<&Objeto> {
        match self.id_por_titulo.get(titulo){
            ...
            None => {println!("Não existem objetos com este título"); vec![]}
        }
    }
    fn procurar_por_autor(&self, autor:&str) -> Vec<&Objeto> {
        match self.id_por_autor.get(autor){
            ...
            None => {println!("Não existem objetos para este autor"); vec![]}
        }
    }
...
impl fmt::Display for Objeto{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self{
            ...
            Objeto::Estatua {titulo, autor, id, palavras_chave} => {
                write!(f, "Estatua: {{Título: {}, autor: {}, id: {}, Palavras-chave: {:?}}}", titulo, autor, id, palavras_chave)
            }
